...
public enum CreditHandler {
	...
	public static boolean allowed(Player player, CreditPurchase credit, int amount) {
		...
		if (player.getCredits() < amount) {
			DialogueManager.sendStatement(player, "@red@You do not have enough Valius Bucks to do this!");
			player.send(new SendMessage("Please visit @red@http://www.valius.org/store/ </col>to purchase more Valius Bucks!"));
			return false;
		}
	...
	public static void spent(Player player, int amount) {
		player.send(new SendMessage("@blu@You have spent " + amount + " Valius Bucks; Remaining: " + player.getCredits() + "."));
		player.getClient().queueOutgoingPacket(new SendString("</col>Valius Bucks: @gre@" + Utility.format(player.getCredits()), 52504));	
